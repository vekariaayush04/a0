...
test("defaults", () => {
  ...
  expect(c.home).toBe("/h/.local/share/sentinel");
...
test("env overrides", () => {
  const c = loadConfig({ HOME: "/h", SENTINEL_PORT: "5000", SENTINEL_HOME: "/x", SENTINEL_CONCURRENCY: "2", SENTINEL_MODEL: "glm-5.3" });
  expect(c.port).toBe(5000); expect(c.home).toBe("/x"); expect(c.concurrency).toBe(2); expect(c.model).toBe("glm-5.3");
});

test("tier overrides", () => {
  const c = loadConfig({ HOME: "/h", SENTINEL_TIER_L3: "kimi-k2.7-code" });
